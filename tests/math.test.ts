...
describe("math functions", () => {
    ...
    it("powIntInv (n=0)", () => {
        ...
        // Strictly positive
        assert.deepEqual(nsf.powIntInv(uint(25, 49), n), nsf.EMPTY);
    ...
    it("powIntInv (n=1)", () => {
        ...
        assert.deepEqual(nsf.powIntInv(uint(-Infinity, -1), n), uint(-Infinity, -1));
        ...
        // Strictly positive
        assert.deepEqual(nsf.powIntInv(uint(25, 49), n), uint(25, 49));

        // Strictly negative
        assert.deepEqual(nsf.powIntInv(uint(-49, -25), n), uint(-49, -25));

        // Positive touching zero
        assert.deepEqual(nsf.powIntInv(uint(0, 49), n), uint(0, 49));

        // Negative touching zero
        assert.deepEqual(nsf.powIntInv(uint(-49, 0), n), uint(-49, 0));
    ...
    it("powIntInv (n=-1)", () => {
        ...
        assert.deepEqual(nsf.powIntInv(uint(-Infinity, 0), n), uint(-Infinity, 0));
        assert.deepEqual(nsf.powIntInv(uint(-Infinity, -1), n), uint(-1, 0));
        ...
        // Strictly positive
        assert.deepEqual(nsf.powIntInv(uint(25, 49), n), uint(prev(1 / 49), next(1 / 25)));

        // Strictly negative
        assert.deepEqual(nsf.powIntInv(uint(-49, -25), n), uint(prev(1 / -25), next(1 / -49)));

        // Positive touching zero
        assert.deepEqual(nsf.powIntInv(uint(0, 49), n), uint(prev(1 / 49), Infinity));

        // Negative touching zero
        assert.deepEqual(nsf.powIntInv(uint(-49, 0), n), uint(-Infinity, next(1 / -49)));

        // Mixed: non degenerate containing zero
        assert.deepEqual(
            nsf.powIntInv(uint(-25, 49), n),
            union([int(-Infinity, next(1 / -25)), int(prev(1 / 49), Infinity)])
        );
        assert.deepEqual(
            nsf.powIntInv(uint(-49, 25), n),
            union([int(-Infinity, next(1 / -49)), int(prev(1 / 25), Infinity)])
        );
    ...
    it("powIntInv (n=-2)", () => {
        ...
        assert.deepEqual(
            nsf.powIntInv(uint(0, 49), n),
            union([
                int(-Infinity, next(1 / prev(-Math.pow(49, next(1 / 2))))),
                int(prev(1 / next(Math.pow(49, next(1 / 2)))), Infinity),
            ])
        );
    ...
    it("powIntInv (n=-3)", () => {
        ...
        assert.deepEqual(
            nsf.powIntInv(uint(0, 49), n),
            uint(prev(1 / next(Math.pow(49, next(1 / 3)))), Infinity)
        );

        // Negative touching zero
        assert.deepEqual(
            nsf.powIntInv(uint(-49, 0), n),
            uint(-Infinity, next(1 / prev(-Math.pow(49, next(1 / 3)))))
        );
        ...
        assert.deepEqual(
            nsf.powIntInv(uint(-25, 49), n),
            union([
                int(-Infinity, next(1 / prev(-Math.pow(25, next(1 / 3))))),
                int(prev(1 / next(Math.pow(49, next(1 / 3)))), Infinity),
            ])
        );
